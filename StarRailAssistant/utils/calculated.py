...
class calculated:
    ...
    def __init__(self, title="崩坏：星穹铁道", det_model_name="ch_PP-OCRv3_det",
                 rec_model_name="densenet_lite_114-fc", number=False, start=True):
        """
        参数: 
        ...
        if start:
            if getattr(sys, 'frozen', None):
                ...
                dir = Path()
            self.ocr = CnOcr(det_model_name=det_model_name, rec_model_name=rec_model_name,
                             det_root=os.path.join(dir, "model/cnocr"),
                             rec_root=os.path.join(dir, "model/cnstd")) if not number else CnOcr(
                det_model_name=det_model_name, rec_model_name=rec_model_name, det_root="./model/cnocr",
                rec_root="./model/cnstd", cand_alphabet='0123456789')
        ...
        for i in range(frequency):
            ...
            while True:
                ...
                if time.time() - start_time > overtime:
                    log.info(("识别超时")) if overtime != 0 else None
                    return False
    ...
    def hsv_click(self, hsv_color, points=(0, 0, 0, 0), offset=(0, 0), flag=True, tolerance=5):
        ...
        while True:
            ...
            log.info(('点击坐标{ret}').format(ret=ret))
            self.click(ret)
            return True
    ...
    def take_screenshot(self, points=(0, 0, 0, 0)):
        ...
        if points != (0, 0, 0, 0):
            ...
            game_img = game_img.crop((game_width / 100 * points[0], game_length / 100 * points[1],
                                      game_width / 100 * points[2], game_length / 100 * points[3]))
    ...
    def scan_screenshot(self, prepared: np, points=None) -> dict:
        ...
        参数：
            ...
            :param pos: 游戏局部截图的坐标
        """
        screenshot, left, top, __, __, game_width, game_length = self.take_screenshot(
            points if points else (0, 0, 0, 0))
    ...
    def part_ocr_other(self, points=(0, 0, 0, 0), debug=False, left=False):
        ...
        out = self.ocr.ocr(img_fp)
        if left:
            data = {i['text']: (int(game_left + x + i['position'][0][0]), int(game_top + y + i['position'][0][1])) for i
                    in out}
        else:
    ...
    def get_pix_rgb(self, desktop_pos: Union[tuple, None] = None, pos: Union[tuple, None] = None,
                    points: tuple = (0, 0, 0, 0)):
        """
        说明：
            获取指定坐标的颜色
    ...
    def switch_window(self, dt=0.1):
        ...
        if len(ws) >= 1:
            ...
            log.info(('没找到窗口{title}').format(title=self.title))
        time.sleep(dt)
