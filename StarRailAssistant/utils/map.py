...
class Map:
    def __init__(self, title=("崩坏：星穹铁道")):
        """
        参数: 
            :param platform: 运行设备
        """
        if sra_config_obj.language != "EN":
            ...
            self.calculated = calculated(title, det_model_name="en_PP-OCRv3_det",
                ...
                if value == 1:  # 进战斗
                    ...
                    if ret == False and map:
                        ...
                        cv.imwrite(f"logs/image/{map_filename}-{date_time}.jpg",
                                   self.calculated.take_screenshot()[0])  # 识别超时,截图
                    ...
                    raise Exception(("map数据错误, fighting参数异常:{map_filename}").format(map_filename=map_filename),
                                    map)
            elif key == "scroll":
                        ...
                        if key == 'map':
                            ...
                            if check and "point" in key and map.split("_")[-1] != "1":
                                ...
                                self.calculated.click_target(
                                    "picture\\orientation_{num}.png".format(num=str(int(key.split("map_")[-1][0]) + 1)),
                                    0.98, map=planet_number)
